...
class Main
  ...
  def run
    loop do
      ...
      answ = gets.to_i
      break if answ == 10 || answ == 0

      answers = [:create_station, :create_train, :add_carriage, :delete_carriage, :set_station, :show_trains_on_stations, :carriage_list,
                 :list_station_trains, :occupy_space]
      begin
        method(answers[answ - 1]).call
      rescue => e
        puts e.message
        retry
      end
  ...
  def get_train
    puts 'Введите номер поезда ххх-хх'
    @train_no = gets
  ...
  def get_station_name
    puts 'Введите название станции Хххх'
    @stan = gets.chomp
